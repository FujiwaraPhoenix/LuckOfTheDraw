...
public class FruitSpeciesGenerator : MonoBehaviour {
    ...
    void Start()
    {
        for (int i = 0; i < 5; i++)
        {
            HealingAmount = Random.Range(-10, 10);
            HungerSaturation = Random.Range(-10, 10);
            SpeedAlteration = Random.Range(-20f, 20f);
            Armor = Random.Range(0, 5);
            RegenLevel = Random.Range(0f, 1f);
            GameObject generatedFruit = Instantiate(FruitPreFab, transform.position, transform.rotation);
            ...
            generatedFruit.GetComponent<SpriteRenderer>().color = Random.ColorHSV(0.0f, 1.0f, 0.0f, 1.0f, 0.0f, 1.0f);
            FruitList[i] = generatedFruit;
            Destroy(generatedFruit);
        }
        Fruit01 = FruitList[0];
        Fruit02 = FruitList[1];
        Fruit03 = FruitList[2];
